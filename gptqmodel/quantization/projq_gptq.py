...
class ProjQ_GPTQ:
    def __init__(self, module: nn.Module, qcfg: Optional[QuantizeConfig] = None,
                 rank: Optional[int] = None, max_iterations: Optional[int] = None):
        self.lock = threading.Lock()
        # Preserve GPTQ's original layer structure parameters
        ...
        # Preserve GPTQ's TP parallel padding info
        ...
        # Preserve GPTQ's device-related configuration
        ...
        # -------------------------- ProjQ parameters ---------------------------------
        # Priority: constructor args > qcfg fields > defaults
        qcfg_rank = getattr(qcfg, "projq_rank", 64) if qcfg else 64
        qcfg_iter = getattr(qcfg, "projq_max_iterations", 5) if qcfg else 5
        self.rank = rank if rank is not None else qcfg_rank
        self.max_iterations = max_iterations if max_iterations is not None else qcfg_iter
        self.tol = 0.02
    ...
    def _clone_module(self, copy=True, device: torch.device = None, target_dtype: torch.dtype = None):
        ...
        if self._tp_pad_cols:
            pad = torch.zeros(
                ...
                dtype=clone.dtype,  # Padding dtype matches weight
                device=clone.device,
            )
            clone = torch.cat((clone, pad), dim=1)
        # Convert to target dtype (priority: external dtype > default float32)
    ...
    def process_batch(self, inp: torch.Tensor) -> Tuple[int, Optional[torch.Tensor], torch.device]:
        ...
        self.calibration_dtype = self.calibration_X.dtype  # Record activation dtype (e.g., FP16)
        # log.debug(
        ...
        # Compute Hessian (fallback to CPU on OOM)
        try:
    ...
    def finalize_hessian(self, target_device: Optional[torch.device] = None) -> torch.Tensor:
        ...
        damp = self.qcfg.damp_percent
        if damp <= 0 or damp >= 1:
            log.warning(f"damp {damp} invalid")
            return
    ...
    def randomized_svd(self, A, rank, oversample=8, n_iter=2):
        ...
        Omega = Omega / torch.sqrt(torch.tensor(r, dtype=compute_dtype, device=A.device))  # Normalize variance
    ...
    def quantize(self, blocksize=128):
        ...
        for t in range(self.max_iterations):
            ...
            (Q, scale, zero, g_idx, duration, full_loss, damp, nsamples) = self.gptq_quantize(
                blocksize=blocksize,
                W=W,
                H=H_perp,
                dead=dead,
            ...
            log.info(f"ProjQ: module {self.name} iteration {t+1} proj_loss: {proj_loss:.6f}")
            full_residual = (W - current_W_hat) @ X_t
            full_loss = float((full_residual.float().norm(p='fro') ** 2).item())
            log.info(f"ProjQ: module {self.name} iteration {t+1} full_loss: {full_loss:.6f}")
    ...
    def gptq_quantize(self, blocksize: int, W: torch.Tensor, H: torch.Tensor, dead: torch.Tensor, target_dtype: torch.dtype) -> Tuple:
        ...
        if self.qcfg.mock_quantization or (self.fail_safe and self.fwd_counter == 0):
            for i1 in range(0, self.columns, blocksize):
                ...
                count = i2 - i1
                W1 = W[:, i1:i2]
                Q1 = torch.zeros_like(W1, dtype=target_dtype)  # Q1 dtype matches activations
                
                # Handle grouped quantization parameters
                ...
                if len(scale) > 0 and len(zero) > 0:
                    ...
                    latest_zero = zero[-1]
                    # Reshape dims for broadcasting (ensure scale/zero dtype matches W1)
                    ...
                    # Symmetric/asymmetric quantization (result cast to target_dtype)
                    if self.qcfg.sym:
