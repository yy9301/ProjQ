...
QUANT_METHOD_FORMAT_MAPPING = {
    ...
    # NOTE: what was this for? forgotten
    # METHOD.GPTQLoRA: {
    #     FORMAT.GPTQ,
    #     FORMAT.GPTQ_V2,
    #     FORMAT.MARLIN,
...
class QuantizeConfig():
    ...
    vram_strategy: VRAMStrategy = field(default=VRAMStrategy.EXCLUSIVE)
     ...
     # ProjQ specific parameters
    projq_rank: int = field(default=8, metadata={"help": "Rank of projection matrix for ProjQ quantization"})
    projq_max_iterations: int = field(default=5, metadata={"help": "Max iterations for ProjQ iterative refinement"})

    # compensation adapter method
    comp_method: str = field(default="eora", metadata={"choices": ["eora", "lordq"]})
    ...
    def __post_init__(self):
        ...
        if self.dynamic is not None:
            self.dynamic = {
                ...
                **{k: v for k, v in self.dynamic.items() if not k.startswith('-')}  # Then add other keys
            }
    ...
    def from_quant_config(cls, quantize_cfg, format: str = None):
        ...
        for key, val in quantize_cfg.items():
            ...
            if key == FORMAT_FIELD_CHECKPOINT:
                ...
                elif val not in {METHOD.GPTQ, METHOD.QQQ, METHOD.AWQ, METHOD.PROJQ}:
        ...
        # ----------------------
        if "projq_rank" in quantize_cfg:
            normalized["projq_rank"] = quantize_cfg["projq_rank"]
        # ----------------------   
    ...
    def to_dict(self):
        out = {
            ...
            "projq_rank": self.projq_rank,  # Add ProjQ rank parameter
