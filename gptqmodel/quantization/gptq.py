...
def debug_log_tensor(name, T, logger=log, max_elems=8):
    try:
        if T is None:
            ...
            return
        # Avoid GPU output blocking; if CUDA, sync may be done first (handled by external caller)
...
class GPTQ:
    ...
    def _compute_hessian_xtx(self, matrix: torch.Tensor) -> torch.Tensor:
        # Compute per-batch second-order statistics xtx = x^T x (partial Hessian result)
        rows = matrix.shape[0]
        if rows == 0:
