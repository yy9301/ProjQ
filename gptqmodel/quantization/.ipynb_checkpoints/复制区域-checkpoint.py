...
class GPTQ:  
    ...
    def quantize(self, blocksize=128):
        ...
        for t in range(self.max_iterations):
            ...
            V_r = V_T.T  # (N, r)            
            P = V_r @ V_r.t()  # (N, N)
            # P = torch.zeros((V_r.shape[0], V_r.shape[0]), device=V_r.device, dtype=V_r.dtype)
            ...
            lambda_proj = 0.3
            H_eff = (1.0 - lambda_proj) * original_H + lambda_proj * H_perp


            # debug_log_tensor("H_perp", H_perp)
            # debug_log_tensor("H_eff", H_eff)
            ...
            # U_new, sigma_new, V_T_new = self.randomized_svd(A=full_residual, rank=r)
            # sigma_new = torch.diag(sigma_new)
            # B = -torch.matmul(U_new, sigma_new)
            # A = torch.matmul(V_T_new, X_pinv)
            # BA_residual = (W - current_W_hat + B @ A) @ X_t
            # BA_loss = float((BA_residual.float().norm(p='fro') ** 2).item())
            # log.info(f"ProjQ: 模块 {self.name} 迭代 {t+1} BA_loss: {BA_loss:.6f}")
            
            
            if t >= 1:    
                # if prev_loss is not None and full_loss > prev_loss:
                #     converged = True
                #     converge_reason = "full loss increased (ppl surrogate)"
                #     break
