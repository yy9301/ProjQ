...
class GPTQ:  
    ...
    def process_batch(self, inp: torch.Tensor) -> Tuple[int, Optional[torch.Tensor], torch.device]:
        ...
        # log.debug(
        #     f"ProjQ: Module {self.name} cached calibration_X. "
        ...
        try:
            ...
            if (
                ...
                log.warn(
                    "ProjQ module '%s' fell back to CPU Hessian accumulation due to GPU OOM during batch processing.",
        ...
        while 0 < damp < 1:
            try:
                ...
                if self.qcfg.damp_auto_increment != 0:
                    log.warn(
                        f"ProjQ: Module `{self.name}` -> Current `damp_percent = {damp:.5f}` is too low, auto-incrementing by `{self.qcfg.damp_auto_increment:.5f}`")
                    damp += self.qcfg.damp_auto_increment
                else:
                    log.warn(
                        f"ProjQ: Module `{self.name}` -> Please increase damp or nsamples for calibration data to avoid the following quant error: current damp_percent=`{damp:.5f}`")
                    raise e
        if not (0 < damp < 1):
            log.error(
                f"ProjQ: Module `{self.name}` -> `damp_percent` must between 0 and 1. current is {damp}. Module cannot be correctly processed.")
            return None, 1.0
        return Hinv, damp
    ...
    def quantize(self, blocksize=128):
        ...
        if self.calibration_X is None or self.calibration_dtype is None:
            raise RuntimeError(f"ProjQ: Calibration data not found for module {self.name}")
        ...
        prev_Q= self.quip_sharp_quantize(
            W=W,
            H=original_H
        )
        # 保持接口兼容
        scale, zero, g_idx = None, None, None
        duration, full_loss, damp, nsamples = 0, 0, 0, self.nsamples
        
        prev_quantized_W = prev_Q.clone()
        X = self.calibration_X  # (N, n)
        X_t = X.t()
        # initial_residual = (W - prev_quantized_W) @ X_t
        # prev_loss = float((initial_residual.to(torch.float32).norm(p='fro') ** 2).item())
        prev_loss = None
        prev_proj_loss = float('inf')
        P_prev = None
        converged = False
        converge_reason = ""
        current_W_hat = prev_quantized_W
        
        # --------------------------- 核心迭代循环 ---------------------------
        for t in range(self.max_iterations):
            log.info(f"----------------iteration{t+1}-------------------")
            R = (W - current_W_hat) @ X_t  # (m, N)
            r = self.rank
            U, sigma, V_T = self.randomized_svd(A=R, rank=r)
            sigma = torch.diag(sigma)
            V_r = V_T.T  # (N, r)            
            P = V_r @ V_r.t()  # (N, N)
            I = torch.eye(P.shape[0], device=X.device,dtype=X_t.dtype)
            X_perp = X_t @ (I - P)  #(n,N)
            debug_log_tensor("X_perp", X_perp)
            
            X_perp_nt = X_perp.T.contiguous()  # (N, in_features)
            H_perp = self.build_hessian_from_X(
                X_new=X_perp_nt,
                target_device=W.device
            )

            # debug_log_tensor("H_perp", H_perp)

            Q = self.quip_sharp_quantize(
                W=W,
                H=H_perp,
                block_size=8,
            )
            ...
            current_W_hat = Q         
            final_residual = (W - current_W_hat) @ X_perp
            proj_loss = float((final_residual.to(torch.float32).norm(p='fro') ** 2).item())
            log.info(f"ProjQ: 模块 {self.name} 迭代 {t+1} proj_loss: {proj_loss:.6f}")
            full_residual = (W - current_W_hat) @ X_t
            full_loss = float((full_residual.float().norm(p='fro') ** 2).item())
            log.info(f"ProjQ: 模块 {self.name} 迭代 {t+1} full_loss: {full_loss:.6f}")
            
            
            if t >= 1:   
                rel_improve = abs(proj_loss - prev_proj_loss) / prev_proj_loss
                if prev_proj_loss != float('inf') and rel_improve < self.tol:
                    converged = True
                    converge_reason = "relative loss improvement < tol"
                    break

                if P_prev is not None:
                    P_diff = torch.norm(P_prev - P, p='fro').item()
                    log.info(f"ProjQ: {self.name} iter {t+1} P_diff: {P_diff:.6f}")
                    if P_diff < self.tol:
                        converged = True
                        converge_reason = "projector stabilized"
                        break

            prev_proj_loss = proj_loss
            prev_loss = full_loss
            P_prev = P.detach()
            
        Q = current_W_hat
        del R, U, sigma, V_T, V_r, X_perp

        if not converged:
            log.warn(f"ProjQ: Module {self.name} did not converge after {self.max_iterations} iterations")
        else:
            log.info(f"ProjQ: Module {self.name} converged - {converge_reason}")
    
        
        duration = time.time() - start
        Q = Q.to(dtype=self.module.weight.dtype)
        return Q, scale, zero, g_idx, duration, full_loss, damp, self.nsamples

    @torch.inference_mode()
    def quip_sharp_quantize(self, W: torch.Tensor, H: torch.Tensor, block_size=8):
        """
        QuIP# (E8 + BlockLDLQ + FWHT)
        """
        ...
        W_pad = torch.zeros((m_pad, n_pad), device=device, dtype=dtype)
        W_pad[:m, :n] = W

        H_pad = torch.zeros((n_pad, n_pad), device=device, dtype=torch.float32)
        H_pad[:n, :n] = H

        # -----------------------------
        # 2. FWHT（代替 Hadamard matrix）
        # -----------------------------
        def fwht(x):
        ...
        def random_sign(x):
            sign = torch.randint(0, 2, x.shape, device=x.device, dtype=torch.float32)
            sign = sign * 2 - 1
            return x * sign

        # incoherence transform
        W_tilde = fwht(random_sign(W_pad))
        W_tilde = fwht(random_sign(W_tilde.T)).T
        debug_log_tensor("W_tilde", W_tilde)

        H_tilde = fwht(random_sign(H_pad))
        H_tilde = fwht(random_sign(H_tilde.T)).T

        # -----------------------------
        # 3. Block LDL (误差传播核心)
        # -----------------------------
        eps = 1e-2
        H_tilde = H_tilde + eps * torch.eye(n_pad, device=device)

        # Cholesky -> 近似 LDL（稳定且快）
        L = torch.linalg.cholesky(H_tilde)

        debug_log_tensor("H_tilde", H_tilde)

        # transform
        Z = W_tilde @ L
        debug_log_tensor("Z", Z)
        # -----------------------------
        # 4. E8 lattice quantization
        # -----------------------------
        def e8_quantize(block):
            if not torch.isfinite(block).all():
                block = torch.nan_to_num(block, nan=0.0, posinf=1e4, neginf=-1e4)
            ...
            if int(s.item()) % 2 != 0:
                idx = torch.argmax(torch.abs(block - r))
                r[idx] += torch.sign(block[idx] - r[idx])
        ...
        for j in range(n_pad):
            ...
            for i in range(0, m_pad, block_size):
                block = z[i:i+block_size]
                ...
                Zq[i:i+block_size, j] = q

                # -------- 关键：误差传播 --------
                err = (block - q).to(torch.float32)

                if j + 1 < n_pad:
                    coeff = L[j, j]
                    if abs(coeff) > 1e-8:
                        Z[i:i+block_size, j+1:] -= (
                            err.unsqueeze(1) * L[j, j+1:].unsqueeze(0)
                        ) / coeff

        # -----------------------------
        # 6. inverse transform
        # -----------------------------
        Wq_tilde = Zq @ torch.inverse(L)

        # inverse FWHT
        def ifwht(x):
            return fwht(x) / x.shape[-1]

        W_hat_pad = ifwht(ifwht(Wq_tilde).T).T
        ...
        if Hinv is not None:
            ...
            if self.nsamples != 0:
                ...
                if math.isnan(avg_loss):
                    ...
                    if self.fail_safe:
                        log.info(f"ProjQ: Failed due to `NaN` loss for `{self.name}`, use mock quantization retry")
