...
# GPTQModel 当前的 Adapter（Lora）是“推理期注入权重”的对象，不是 PyTorch 意义上的可训练模块。
 # lora_A / lora_B 不会被注册为模型参数,lora_A / lora_B 是普通 Tensor
 # model.parameters() 根本看不到它们
 # optimizer 即使拿到 Tensor，也不会追踪梯度
...
class Lora(Adapter, nn.Module):
    """
    修改说明：
    1. 新增 enable_training 开关
       - False：保持原始 Tensor 行为
       - True：把 lora_A/lora_B 转为 nn.Parameter，可训练
    2. 支持 save_pretrained / load 功能
    """
    def __init__(
        self,
        rank: int,
        ...
        # 如果 enable_training=True，把 A/B 变成 Parameter
        if enable_training:
    ...
    def post_init(self, weight_key: str, device:torch.device, lora_A: torch.Tensor=None, lora_B: torch.Tensor=None):
        ...
        # enable_training 决定是否注册为 Parameter
