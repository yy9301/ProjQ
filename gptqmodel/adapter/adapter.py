...
# GPTQModel's current Adapter (LoRA) is an “inference-time weight injection” object, not a trainable module in PyTorch's sense.
 # lora_A / lora_B are not registered as model parameters; they are plain Tensors
 # model.parameters() cannot see them at all
 # optimizer will not track gradients even if it receives the Tensors

# import pcre as re

# Current (modified) adapter.py
import re
...
class Lora(Adapter, nn.Module):
    """
    Modification notes:
    1. Added enable_training switch
       - False: keep original Tensor behavior
       - True: convert lora_A/lora_B to nn.Parameter, making them trainable
    2. Supports save_pretrained / load functionality
    """
    def __init__(
        self,
        rank: int,
        ...
        Adapter.__init__(self, rank, path)
        nn.Module.__init__(self)

        self.enable_training = enable_training

        # If enable_training=True, convert A/B to Parameters
        if enable_training:
            self.lora_A = nn.Parameter(lora_A) if lora_A is not None else None
            self.lora_B = nn.Parameter(lora_B) if lora_B is not None else None
        else:
            self.lora_A = lora_A
            self.lora_B = lora_B
    ...
    def apply(self, x: torch.Tensor, out: torch.Tensor) -> torch.Tensor:
        ...
        A_cast = A.to(dtype=x.dtype)
        B_cast = B.to(dtype=x.dtype)
        # print(x.shape, A_cast.shape, B_cast.shape)
        if out.dim() > x.dim() and out.shape[0] > 1:
            out_orgi_shape = out.shape
            out = out.view(-1, out.shape[-1])
            out.add_((x @ A_cast) @ B_cast)
            out = out.view(out_orgi_shape)
            return out
        else:
            return out.add_((x @ A_cast) @ B_cast)
    ...
    def post_init(self, weight_key: str, device:torch.device, lora_A: torch.Tensor=None, lora_B: torch.Tensor=None):
        ...
        # enable_training decides whether to register as Parameter
        if self.enable_training:
            self.lora_A = nn.Parameter(lora_A.to(device=device))
            self.lora_B = nn.Parameter(lora_B.to(device=device))
        else:
            self.lora_A = lora_A.to(device=device)
            self.lora_B = lora_B.to(device=device)

        # print(weight_key,self.lora_A.shape, self.lora_B.shape)
