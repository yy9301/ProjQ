...
# Added code for importing activation here
import copy
import time
...
from ..eora.lordq import lordq_compute_lora, lordq_process_input, merge_lordq_segments
...
class EoraProcessor(LoopProcessor):
    def __init__(self, tokenizer, qcfg: QuantizeConfig, calibration, prepare_dataset_func,
        ...
        # comp method dispatch: eora / lordq
        self.comp_method = getattr(qcfg, "comp_method", "eora")
        if self.comp_method == "lordq":
            self.eora_compute_lora = lordq_compute_lora
            self.eora_process_input = lordq_process_input
            self.merge_segments = merge_lordq_segments
        else:
            self.merge_segments = merge_eora_segments
    ...
    def pre_process_fwd_hook(self, name: str) -> Callable:
        def tmp(module, input: Tuple[torch.Tensor, ...], output: torch.Tensor):
            # Save Activation (assume first input tensor)
    ...
    def _finalize_eigen_scaling_matrix(self, name: str) -> torch.Tensor:
        ...
        return self.merge_segments(segment_pairs)
...
# =============================Modification==============================
        activation_tensor = self._current_activations.pop(module.name, None)
        if activation_tensor is None:
            raise RuntimeError(f"Activation for module {module.name} not found!")

        A, B = self.eora_compute_lora(
            w_wq_delta=w_wq_delta,
            X=activation_tensor,  # New parameter
            name=module.name,
