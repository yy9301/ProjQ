...
from ..looper.projq_processor import ProjQProcessor
...
def debug_log_tensor(name, T, logger=log, max_elems=8):
    try:
        ...
        # Convert to float32 for statistics (avoid precision issues)
        T_float = T.detach().to(dtype=torch.float32)
        finite_mask = torch.isfinite(T_float)

        # Outlier check
        any_nan = torch.isnan(T_float).any().item()
        any_inf = torch.isinf(T_float).any().item()
        any_nonfinite = not finite_mask.all().item()
        anomaly_info = (
            f"Outlier check:\n"
            f"  - Contains NaN: {any_nan}\n"
            f"  - Contains Inf: {any_inf}\n"
            f"  - Contains non-finite values: {any_nonfinite}"
        )

        # Statistical info (min, max, mean, norm)
        stats_info = ""
        try:
            stats = (
                f" Statistics:\n"
                f"  - Min: {float(torch.min(T_float).item()):.6g}\n"
                f"  - Max: {float(torch.max(T_float).item()):.6g}\n"
                f"  - Mean: {float(torch.mean(T_float).item()):.6g}\n"
                f"  - Norm: {float(torch.norm(T_float).item()):.6g}"
            )
            stats_info = stats
        except Exception as e:
            stats_info = f" Statistics computation failed: {str(e)}"

        # First N elements
        flat = T_float.flatten()
        head = flat[:max_elems].cpu().numpy().tolist()
        elements_info = f" First {max_elems} elements: {head}"

        # Join all info and log
        log_message = "\n".join([base_info, anomaly_info, stats_info, elements_info])
        logger.info(log_message)

    except Exception as e:
        logger.info(f" {name} log print failed: {str(e)}")
    ...
    for layer_index in pb:
        ...
        for p_index, processor in enumerate(looper.processors):
            ...
            if not is_last_module and processor.fwd_after_process and subset_context is not None:
                ...
                #     )
# After quantization, forward pass via looper._run_forward_batches to generate current layer's output
# Output layer_outputs is passed to next layer as input (via processor.receive_layer_inputs)
                try:
            ...
            if p_index == len(looper.processors) - 1:
                ...
                if region_timer is not None:
                    ...
                    # -----------------------------------
            # if layer_outputs:
            #     for batch_idx, batch_output in enumerate(layer_outputs):
            #         for tensor_idx, output_tensor in enumerate(batch_output):
            #             debug_log_tensor(
            #                 f"Layer {layer_index} output - batch {batch_idx} tensor {tensor_idx}",
            #                 output_tensor,
            #                 logger=log
            #             )
# ---------------------------------------
            if processor.fwd_after_process:
                ...
                # Pass current layer output as next layer input
                processor.receive_layer_inputs(layer_outputs)
                layer_inputs = processor.inputs_cache.layer_inputs # Update input cache
