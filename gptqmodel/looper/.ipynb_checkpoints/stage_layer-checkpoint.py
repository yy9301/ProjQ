...
def debug_log_tensor(name, T, logger=log, max_elems=8):
    try:
        ...
        try:
            stats = (
                f" 统计信息:\n"
                f"  - 最小值: {float(torch.min(T_float).item()):.6g}\n"
                f"  - 最大值: {float(torch.max(T_float).item()):.6g}\n"
                f"  - 均值: {float(torch.mean(T_float).item()):.6g}\n"
                f"  - 范数: {float(torch.norm(T_float).item()):.6g}"
            )
            stats_info = stats
        except Exception as e:
            stats_info = f" 统计信息计算失败: {str(e)}"

        # 前N个元素
        flat = T_float.flatten()
        head = flat[:max_elems].cpu().numpy().tolist()
        elements_info = f" 前{max_elems}个元素: {head}"
        ...
        logger.info(f" {name} 日志打印失败: {str(e)}")
    ...
    for layer_index in pb:
        ...
        for p_index, processor in enumerate(looper.processors):
            ...
            if not is_last_module and processor.fwd_after_process and subset_context is not None:
                ...
                #     )
# 量化处理后，通过 looper._run_forward_batches 执行前向传播，生成当前层的输出
# 输出结果 layer_outputs 会被传递给下一层作为输入（通过 processor.receive_layer_inputs）
                try:
...
# ---------------------------------------
            if processor.fwd_after_process:
                ...
                layer_inputs = processor.inputs_cache.layer_inputs # 更新输入缓存
