...
class AWQProcessor(LoopProcessor):
    ...
    def init_quant(self):
        ...
        class Catcher(nn.Module):
            def __init__(self, module):
                super().__init__()
                self.module = module
                # Forward attention_type attribute (added this line)
                self.attention_type = module.attention_type  # Key: expose the original layer's attention_type
    ...
    def _search_best_scale(
        ...
        # Add check to allow quantization when group_size=-1
        if self.qcfg.group_size is None or self.qcfg.group_size <= 0:
    ...
    def layer_quantize(self, module: Module, device: torch.device, named_childs: Dict[str, NamedModule]):
        ...
        # Transformers >= 4.48.0 requires positional embeddings should be computed before forward pass
        if self.module_kwargs.get("position_embeddings") is None:
            if hasattr(self.model.model, "rotary_emb") and callable(self.model.model.rotary_emb):
                # Only call when model has rotary_emb
                self.module_kwargs["position_embeddings"] = self.model.model.rotary_emb(
                    self.inps, self.module_kwargs["position_ids"]
                )
            else:
                # For models without rotary embedding, set to None
                self.module_kwargs["position_embeddings"] = None
    ...
    def _compute_best_clip(
        ...
        # Add check to allow quantization when group_size=-1
        if self.qcfg.group_size is None or self.qcfg.group_size <= 0:
            group_size = org_w_shape[1]  # treat as full per-tensor group
        else:
    ...
    def pseudo_quantize_tensor(self, w: torch.Tensor):
        org_w_shape = w.shape
        # When group_size=-1 we don't want reshape to fail
        if self.qcfg.group_size is None or self.qcfg.group_size <= 0:
            w = w.reshape(1, -1)  # treat entire tensor as one group
        else:
        ...
        if self.qcfg.zero_point:
            ...
            # Allow groupsize=-1 to work
            if zeros.numel() == 1:
                # per-tensor quantization (group_size = -1): broadcast scalar zero point
                zeros = zeros.expand(org_w_shape[0], 1)
            else:
        ...
        # Allow groupsize=-1 to work
        if scales.numel() == 1:
            scales = scales.expand(org_w_shape[0], 1)
        else:
