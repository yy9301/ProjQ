...
class ProjQProcessor(LoopProcessor):
    def __init__(self, tokenizer, qcfg: QuantizeConfig, calibration, prepare_dataset_func,
        ...
        self.max_iterations = getattr(qcfg, "projq_max_iterations", 5)
    ...
    def set_calibration_dataset(self, calibration_dataset):
        raise NotImplementedError("ProjQProcessor's calibration_dataset cannot be modified")

    def preprocess(self, module: NamedModule, fail_safe: bool=False):
        ...
        # log.info(f"[ProjQ] {module.full_name} effective params: bits={qcfg_clone.bits}, group_size={qcfg_clone.group_size}, rank={self.rank}")
    ...
    def process(self, module: NamedModule):
        ...
        if expected_device is not None:
            ...
            if g_weight is not None:
                assert g_weight.device == expected_device, (
                    f"ProjQ task for module '{module.full_name}' expected device {expected_device}, "
                    f"but found weight on {g_weight.device}."
                )
                assert g_weight.data.device == expected_device, (
                    f"ProjQ task for module '{module.full_name}' weight.data on {g_weight.data.device} "
            ...
            if g_h is not None:
                assert torch.device(g_h.device) == expected_device, (
                    f"ProjQ Hessian tensor for '{module.full_name}' lives on {g_h.device}, expected {expected_device}."
                )
        ...
        #ProjQ[name].layer.weight.data = q_full_weight.to(device=ProjQ[name].device)
        ...
        with self.lock:
            ...
            # logger.info(f"Quantizing module END: {name}, {ProjQ[name].shape()}")
            if self.calculate_w_wq_diff:
                module.state.update({
    ...
    def finalize(self, model: BaseQModel, **kwargs):
        ...
        model.quantize_config.quant_method = METHOD.PROJQ

        super().finalize(model=model, **kwargs)

    def verify_calibration_dataset(self, processor_index: int) -> bool:
        if self.calibration_dataset is None:
            raise ValueError("ProjQProcessor's calibration_dataset must be provided.")
        else:
            return True
    ...
    def name(self) -> str:
        ...
        return f"projq (rank={self.rank}, max_iter={self.max_iterations})"
