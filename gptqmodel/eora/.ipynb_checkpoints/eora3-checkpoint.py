...
def eora_compute_lora(
    ...
    truc_u1, truc_s1, truc_v1 = randomized_svd(A=RY+jitter,rank=half_rank,oversample=half_rank+5,n_iter=2)

    # lowrank_r = rank
    # truc_s = S[:lowrank_r]
    # truc_u = U[:, :lowrank_r]
    # truc_v = V[:lowrank_r, :]
    truc_sigma1 = torch.diag(truc_s1)

    B1 = torch.matmul(truc_u1, truc_sigma1).to(dtype=dtype) # default to float16, check if we should save to float32
    A1 = torch.matmul(truc_v1, Y_inv).to(dtype=dtype) # default to float16, check if we should save to float32 
    
    R_new = w_wq_delta + B1 @ A1
    debug_log_tensor(R_new, torch.eye(R_new.shape[1], device=R_new.device), "Residual_R_new")

    truc_u2, truc_s2, truc_v2 = randomized_svd(A=R_new,rank=half_rank,oversample=half_rank+5,n_iter=2)

    # lowrank_r = rank
    # truc_s = S[:lowrank_r]
    # truc_u = U[:, :lowrank_r]
    # truc_v = V[:lowrank_r, :]
    truc_sigma2 = torch.diag(torch.sqrt(truc_s2))

    B2 = torch.matmul(truc_u2, truc_sigma2).to(dtype=dtype) # default to float16, check if we should save to float32
    A2 = torch.matmul(truc_sigma2, truc_v2).to(dtype=dtype) # default to float16, check if we should save to float32 
    
    B = torch.cat([B1, B2], dim=1)   # (out_features, 64)
    A = torch.cat([A1, A2], dim=0)   # (64, in_features)

    
    debug_log_tensor(B1, A1, "B1A1_first_stage")
    debug_log_tensor(B2, A2, "B2A2_second_stage")
    debug_log_tensor(B, A, "B_final_A_final")
