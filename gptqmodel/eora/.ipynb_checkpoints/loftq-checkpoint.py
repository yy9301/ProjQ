...
def eora_compute_lora(
    ...
    truc_u, truc_s, truc_v = randomized_svd(A=w_wq_delta,rank=rank,oversample=rank+5,n_iter=2)
    ...
    A = torch.matmul(truc_sigma, truc_v).to(dtype=dtype) # default to float16, check if we should save to float32
