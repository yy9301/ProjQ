...
class BaseQuantLinear(nn.Module):
    ...
    def __init__(self,
        ...
        if adapter is not None:
            adapter_copy = copy.deepcopy(adapter)  # deepcopy ensures each layer is independent
            enable_training = getattr(adapter_copy, "enable_training", False)

            if enable_training:
                if not isinstance(adapter_copy, nn.Module):
                    raise TypeError("Trainable adapter must inherit from nn.Module")
                self.add_module("adapter", adapter_copy)  # Register deepcopy'd adapter
            self.adapter = adapter_copy
        else:
            self.adapter = None
        # ----------------------------------------------
