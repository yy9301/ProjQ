...
#         # Token-id level comparison instead of string
...
def eval_boolq(model, tokenizer, dataset, dev, max_length=512):
    ...
    for example in tqdm(dataset):
        ...
        for opt in options:
            # Build prompt: Question + Passage + Answer
...
def eval_gsm8k(model, tokenizer, dataset, dev, batch_size=16):
    ...
    for ex in dataset:
        ...
        questions.append(q + QUESTION_PROMPT)
        answers.append(a)
    ...
    for i in tqdm(range(steps)):
        ...
        for text in decoded:
            ...
            seg = text.split(ANSWER_PROMPT)

            if len(seg) > 1:

                ans = re.findall(r'-?\d+\.?\d*', seg[1])

                if len(ans) > 0:
                    preds.append(float(ans[0]))
                else:
                    preds.append(float(nums[-1]))
